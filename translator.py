...
class PDFTranslator:
    ...
    def translate_pdf(self,texts: List[str]) -> List[str]:
        return [f"\033[30m\033[1m第 {index + 1}页的翻译\033[0m\033[0m\n" + self.model.make_request(self.prompt + text)[0] for index,text in enumerate(texts)]
    model = OpenAIModel(model="gpt-3.5-turbo")
    pdfcleaner = PaperCleaner(path="2.pdf")
    translator = PDFTranslator(model)
    for i in translator.translate_pdf(pdfcleaner.cleaned_text) :
        print(i)
